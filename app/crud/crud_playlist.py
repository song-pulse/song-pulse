...
class CRUDPlaylist(CRUDBase[Playlist, PlaylistCreate, PlaylistUpdate]):
    ...
    def create_or_modify_with_participant(self, db_session: Session, obj_in: PlaylistCreate, participant_id: int) -> Playlist:
        obj_in_data = jsonable_encoder(obj_in)
        fresh_playlist = self.model(**obj_in_data, participant_id=participant_id)

        existing = self.get_by_participant_and_type(db_session, participant_id, obj_in.type)
        if existing:
            self.update(db_session, db_obj=existing, obj_in=fresh_playlist) #TODO how does this work??
        else:
